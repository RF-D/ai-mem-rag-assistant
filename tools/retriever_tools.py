...
def retriever_tool_meta(vectorstore):
    def retrieve_documents(query):
        ...
        docs = vectorstore.similarity_search(query, k=20)  # Adjust the value of k as needed
        ...
        reranker = VoyageAIRerank(
            ...
            top_k=8  # Adjust the value of top_k as needed
        )

        # Create a ContextualCompressionRetriever with the reranker
        compression_retriever = ContextualCompressionRetriever(
            base_compressor=reranker,
            base_retriever=vectorstore.as_retriever(search_kwargs={"k": 20})  # Adjust the value of k as needed
        )
